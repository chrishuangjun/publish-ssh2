...
const help = function () {
    ...
    console.log()
    console.log(chalk.green('    publish-ssh2-cli gvc(或者ganeratorVueComp) [dist]'))
    console.log()
    console.log(chalk.gray('    生成部署配置文件,命令如下：'))
    console.log()
    console.log(chalk.green('    publish-ssh2-cli gcfg(或者generator_config) [cfgPath]'))
    console.log()
    console.log(chalk.grey('    根据部署配置文件说明填写信息，然后部署代码，命令如下：'))
    console.log()
    console.log(chalk.green('    publish-ssh2-cli p(或者publish) [cfgPath]'))
    console.log()
    console.log(chalk.grey('    部署完成后，发现有问题，马上可以通过命令恢复至上个版本，命令如下：'))
    console.log()
    console.log(chalk.green('    publish-ssh2-cli rv(或者revert) [cfgPath]'))
    console.log()
    console.log(chalk.grey('    初始化项目，命令内置了两个模板一个是H5（移动端开发模板），一个是PC(pc端开发模板)，命令如下：'))
    console.log()
    console.log(chalk.green('    publish-ssh2-cli i(或者init) [repoUrl]'))
    console.log()
    console.log(chalk.grey('    将指定文件夹生成压缩包，命令如下：'))
    console.log()
    console.log(chalk.green('    publish-ssh2-cli c(或者compress) [cfgPath]'))
}
